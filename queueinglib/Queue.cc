...
void Queue::handleMessage(cMessage *msg)
{
    if (msg == endServiceMsg) {
        ...
        if (queue.isEmpty()) {
           ...
           arrivalTime.record(msg->getArrivalTime());
...
simtime_t Queue::startService(Job *job)
{
    ...
    cMsgPar * par;
    par = &(job->par("Tempo di servizio"));
    parServiceTime =par->doubleValue();
    */

    double val1=job->par("Tempo di servizio").doubleValue();

    //std::cout<<"VALORE UE (START) : " <<(val1)<< std::endl;
